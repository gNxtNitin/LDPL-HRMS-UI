...
$(document).ready(() => {
  ...
  `;
    ...
    daReport_table = $('.datatables-dareport').DataTable({
        ...
        ajax: {
            url: `${baseUrl}/leave/GetTeamLeaveReport`,
            dataSrc: ''
            //data: {
            //    EmpId: '',
            //    DTRangeFrom: '',
        ...
        columns: [
            {
                ...
                render: (data, type, row) => `
                    <input type="checkbox" class="form-check-input row-checkbox" value="${row.AUTO_KEY_REF}_${row.EMP_CODE}" />
                `
            },

            { data: 'AUTO_KEY_REF', title: 'ID' },
            { data: 'EMP_CODE', title: 'EMP ID' },
        ...
        columnDefs: [
            ...
            //},
            {
                ...
                render: function (data, type, full, meta) {
                    console.log(full);
                    const daid = full.AUTO_KEY_REF;
                    const empid = full.EMP_CODE;
                    return `
                    <div class="d-flex align-items-center">
    ...
    $('.datatables-dareport tbody').on('click', '.approve-da, .reject-da', function () {
        ...
        $.ajax({
            url: `${baseUrl}/leave/ApproveTeamLeave`,
            type: 'POST',
            ...
            success: function (resp) {
                if (resp.code > 0) {
                    ToastWrapper.success(`Leave ${isApproved ? 'approved' : 'rejected'} successfully.`);
    ...
    function handleMultiAction(actionType) {
        ...
        $.ajax({
            url: `${baseUrl}/leave/MultiApproveTeamLeave`,
            method: 'POST',
            contentType: 'application/json',
            ...
            success: function (res) {
                if (res.status === 200) {
                    ...
                    if (res.success === total) {
                        ToastWrapper.success("Leave request successfully updated");
                    }
